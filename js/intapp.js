/* intapp.js — wiring for Integrator 1.0 (platinum).
 * Same trainer brain as the differentiator: random pool, streaks,
 * tickets, tiny sounds. One difference: answers are graded by
 * differentiation — an answer counts when its derivative matches
 * the integrand, so any equal antiderivative counts and +C is
 * simply omitted.
 */
(function () {
  "use strict";
  ...
  function newProblem() {
    ...
    state.problem = IntProblems.generate(els.range.value);
    state.count++;
    state.locked = false;
  ...
  function check() {
    ...
    var ok;
    try {
      ok = IntProblems.check(raw, state.problem) === "good";
    } catch (e) {
  ...
  function reveal() {
    ...
    els.ticketBody.innerHTML = math("∫" + Tex.ofSource(state.problem.source) +
      ...
      "<br>Any equal form is accepted (+C omitted).";
