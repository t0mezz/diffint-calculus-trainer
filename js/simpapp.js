/* simpapp.js — wiring for Simplifier 1.0 (platinum).
 * Same trainer brain as the differentiator: random pool, numeric
 * equivalence checking, streaks, tickets, tiny sounds. One extra
 * rule: an answer must be SMALLER than the problem — an equal form
 * that simplifies nothing does not count.
 */
(function () {
  "use strict";
  ...
  function newProblem() {
    ...
    state.problem = SimProblems.generate(els.range.value);
    state.count++;
    state.locked = false;
  ...
  function check() {
    ...
    var verdict;
    try {
      verdict = SimProblems.check(raw, state.problem);
    } catch (e) {
    ...
    if (verdict === "good") {
      ...
      say(state.streak >= 5 ? "Correct — " + state.streak + " in a row."
        ...
        : "Correct.", "good");
    } else if (verdict === "unsimplified") {
      state.streak = 0;
      blip(140, 0.18, "sawtooth", 0.07);
      say("Equal, but not simpler yet — keep going.", "bad");
    } else {
      state.streak = 0;
      blip(140, 0.18, "sawtooth", 0.07);
  ...
  function reveal() {
    ...
    els.ticketBody.innerHTML = math(Tex.ofSource(state.problem.source)) +
      " &nbsp;→&nbsp; " + math(Tex.render(state.problem.expectedAst)) +
      "<br>Any simpler equal form is accepted.";
